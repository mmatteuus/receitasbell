...
async function run() {
  ...
  const baseUrl = String(process.env.BASEROW_API_URL || "https://api.baserow.io").replace(/\/+$/, "");
  const token = String(process.env.BASEROW_API_TOKEN || "").trim();
  const tableId = String(process.env.BASEROW_TABLE_USERS || "").trim();

  if (!userId || !password) {
    throw new Error("ADMIN_USER_ID e ADMIN_NEW_PASSWORD obrigatorios.");
  }
  if (!token) {
    throw new Error("BASEROW_API_TOKEN obrigatorio.");
  }
  if (!tableId) {
    throw new Error("BASEROW_TABLE_USERS obrigatorio.");
  }
  ...
  const response = await fetch(
    `${baseUrl}/api/database/rows/table/${tableId}/${encodeURIComponent(userId)}/?user_field_names=true`,
    {
      method: "PATCH",
      headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Baserow HTTP ${response.status}: ${message}`);
  }
