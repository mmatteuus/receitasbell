...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    if (errorParam) { response.redirect(302, appendQuery(fallback, "error", "stripe_oauth_denied")); return; }
    if (!code || !state) { response.redirect(302, appendQuery(fallback, "error", "stripe_oauth_failed")); return; }
    const { handleStripeOAuthCallback } = await import("../../src/server/integrations/stripe/oauth.js");
