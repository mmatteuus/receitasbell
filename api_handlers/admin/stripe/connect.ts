...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  return withApiHandler(request, response, async ({ requestId }) => {
    ...
    const body = (typeof request.body === "string" ? JSON.parse(request.body || "{}") : (request.body ?? {})) as { returnTo?: string | null };
    const tenantUserId = access.type === "session" ? access.userId : "admin-api";
    const result = await getStripeConnectUrl(tenant.id, { tenantUserId, returnTo: body.returnTo || "/admin/pagamentos/configuracoes" });
