...
test.describe('ReceitasBell user flows', () => {
  ...
  test('favoritos persistem para o usuario identificado', async ({ page }) => {
    ...
    try {
      ...
      await expect(
        page.getByRole('link', { name: recipeTitle, exact: true }).first()
      ).toBeVisible();
    } finally {
  ...
  test('checkout desbloqueia receita paga e o pagamento abre no admin', async ({ page }) => {
    ...
    try {
      ...
      }>(page, '/api/payments/checkout/session', {
        method: 'POST',
        body: {
          recipeIds: [recipe.id],
      ...
      expect(checkoutResponse.body.paymentOrderId).toBeTruthy();
      expect(checkoutResponse.body.checkoutUrl).toBeTruthy();

      await openRoute(
        page,
        `/compra/sucesso?slug=${recipe.slug}&status=approved&payment_id=${checkoutResponse.body.paymentOrderId}&count=1`
      );
      await page.waitForLoadState('networkidle');
      ...
      expect(paymentId).toBe(checkoutResponse.body.paymentOrderId);
      ...
      await page
        .getByPlaceholder('Adicione uma nota interna sobre este pagamento...')
        .fill(noteText);
  ...
  test('admin lista a receita e abre o editor preenchido', async ({ page }) => {
    ...
    try {
      ...
      await expect(
        page.getByText('O slug é gerado a partir do título e fica fixo após a primeira publicação.')
      ).toBeVisible();
      await expect(fieldByLabel(page, 'Descrição', 'textarea')).toHaveValue(
        'Receita criada para validar o admin.'
      );
    } finally {
      await deleteRecipeFixture(page, recipe.id);
    }
