...
import { pickFeaturedRecipes, pickPremiumRecipes } from "../src/lib/home/curation";
...
test("getRecipeImage resolve prioridade imageUrl -> image -> placeholder", async () => {
  expect(getRecipeImage(makeRecipe({ imageUrl: "https://img/a.jpg", image: "https://img/legacy.jpg" }))).toBe("https://img/a.jpg");
  expect(getRecipeImage(makeRecipe({ imageUrl: "", image: "https://img/legacy.jpg" }))).toBe("https://img/legacy.jpg");
...
test("getRecipePresentation melhora titulo genérico", async () => {
  const recipe = makeRecipe({ title: "Receita Premium 123456", categorySlug: "doces", tags: ["premium"] });
