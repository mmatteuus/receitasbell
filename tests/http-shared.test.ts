...
import { getQueryValue, json, readJsonBody, setPublicCache } from "../src/server/shared/http.js";
...
describe("shared/http helpers", () => {
  ...
  test("getQueryValue le de req.query e fallback de URL", () => {
    const fromQuery = getQueryValue({ query: { slug: "bolo" }, headers: {}, url: "/x" } as unknown as VercelRequest, "slug");
    expect(fromQuery).toBe("bolo");

    const fromUrl = getQueryValue(
      { query: {}, headers: { host: "localhost" }, url: "/x?slug=torta" } as unknown as VercelRequest,
      "slug",
    );
  ...
  test("readJsonBody parseia string json", async () => {
    const body = await readJsonBody<{ foo: string }>({ body: '{"foo":"bar"}' } as unknown as VercelRequest);
