...
import { runConsistencyJob } from '../../src/server/domains/jobs/maintenance.js';
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  return withApiHandler(request, response, async (log) => {
    ...
    if (authHeader !== expectedToken && querySecret !== cronSecret) {
      log.warn("Unauthorized attempt to run consistency job", { 
    ...
    log.info("Consistency job started");
