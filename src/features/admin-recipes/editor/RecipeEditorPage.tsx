...
export default function RecipeEditorPage() {
  ...
  const pageTitle = isEditing ? 'Editar Receita' : 'Nova Receita';
  const pageHead = (
    <PageHead
      title={pageTitle}
      description="Gerencie o conteúdo e o SEO das receitas direto do painel."
      noindex={true}
    />
  );
    ...
    <>
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="font-heading text-2xl font-bold sm:text-3xl">{pageTitle}</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              O formulário publica pela camada de API e repositórios, mantendo a UI isolada da
              persistência.
            </p>
            {isOffline && (
