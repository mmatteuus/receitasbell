...
export function StripeConnectCard({
  ...
  const canConnect = Boolean(settings?.oauthConfigured ?? true);
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <CardDescription className="text-xs">
                {isConnected
                  ? "Conta vinculada com sucesso — pronta para receber pagamentos."
                  : "Conecte sua conta Stripe para receber pagamentos de forma segura."}
              </CardDescription>
            </div>
          </div>
          ...
          {isConnected && (
            <div className="hidden sm:flex flex-col items-end">
              <span className="text-[10px] font-bold uppercase tracking-widest text-green-600 bg-green-100 dark:bg-green-900/40 px-2 py-0.5 rounded">
                Ativo
              </span>
            </div>
          )}
          {reconnectRequired && !isConnected && (
            <div className="hidden sm:flex flex-col items-end">
              <span className="text-[10px] font-bold uppercase tracking-widest text-amber-600 bg-amber-100 dark:bg-amber-900/40 px-2 py-0.5 rounded">
                Atenção
              </span>
            </div>
          )}
          ...
          <div className="flex items-start gap-3 rounded-xl border border-amber-400/30 bg-amber-400/5 p-4 text-sm text-amber-800 dark:text-amber-200">
            ...
            <div>
              ...
              <p className="mt-1 text-xs opacity-90 leading-relaxed">
                A integração com o Stripe ainda não foi configurada pelo
                administrador da plataforma. Entre em contato com o suporte.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4 pt-2">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div className="flex items-center gap-3 rounded-xl border bg-card/50 p-3 text-xs text-muted-foreground">
                <Zap aria-hidden="true" className="h-4 w-4 text-violet-500" />
                {settings?.userId ? `Conta: ${settings.userId}` : "Checkout Stripe"}
              </div>
            </div>

            <div className="flex items-center gap-2 text-[10px] text-muted-foreground px-1">
              <Network aria-hidden="true" className="h-3 w-3" />
              <span>Conexão estável estabelecida com a API do Stripe</span>
            </div>
            ...
            {onDisconnect && (
              <Button
                ...
                aria-label="Desconectar conta Stripe"
              >
                {disconnecting ? (
                ...
                {disconnecting ? "Desconectando..." : "Desconectar conta Stripe"}
              </Button>
            )}
          </div>
        ) : (
          <div className="space-y-4">
            <Button
              ...
              className={cn(
                ...
                canConnect
                  ? "bg-[#635BFF] hover:bg-[#5851DB] hover:shadow-[#635BFF]/20"
            ...
            >
              ...
              <span className="font-bold tracking-tight">
                {reconnectRequired ? "Reconectar ao Stripe" : "Conectar com Stripe"}
              </span>
            </Button>
          </div>
        )}
