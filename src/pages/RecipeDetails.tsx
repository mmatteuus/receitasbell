...
export default function RecipeDetails() {
  ...
  const scaleIngredient = (text: string) => {
    ...
    const units: Record<string, string> = {
      "xícara": "xícaras", "colher": "colheres", "dente": "dentes", "fatia": "fatias",
      "lata": "latas", "caixa": "caixas", "pacote": "pacotes", "vidro": "vidros",
      "copo": "copos", "pitada": "pitadas", "unidade": "unidades", "peça": "peças",
      "folha": "folhas", "maço": "maços", "ramo": "ramos"
    };
    const pluralUnits = Object.fromEntries(Object.entries(units).map(([k, v]) => [v, k]));

    let newRest = rest;
    const unitMatch = rest.match(/^(\s*)([a-záàâãéèêíïóôõöúçñ]+)/i);

    if (unitMatch) {
      const [_, space, word] = unitMatch;
      const lowerWord = word.toLowerCase();

      if (newValue > 1 && units[lowerWord]) {
        newRest = space + units[lowerWord] + rest.substring(space.length + word.length);
      } else if (newValue <= 1 && pluralUnits[lowerWord]) {
        newRest = space + pluralUnits[lowerWord] + rest.substring(space.length + word.length);
      }
    }

    return `${formattedValue}${newRest}`;
  };
  ...
  // Determina quais ingredientes/instruções mostrar (Teaser ou Completo)
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <Button 
                    ...
                    onClick={() => setCustomServings(s => Math.max(1, s - 1))}
                  >
                  ...
                  <Button 
                    ...
                    size="icon" 
                    className="h-6 w-6 print:hidden" 
                    onClick={() => setCustomServings(s => s + 1)}
                  >
                ...
                {customServings !== recipe.servings && (
                  <Button 
                    variant="link" 
                    size="sm" 
                    className="h-auto p-0 text-xs text-muted-foreground hover:text-orange-600 mt-1 print:hidden"
                    onClick={() => setCustomServings(recipe.servings)}
                  >
                    Resetar Porções
                  </Button>
                )}
          ...
          <div className="space-y-6">
            {instructions.map((step, index) => (
              <div key={index} className="group relative flex gap-4">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-primary/30 bg-background text-sm font-bold text-primary shadow-sm group-hover:border-primary group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
                  {index + 1}
                </div>
                <p className="pt-1 text-base leading-relaxed text-muted-foreground group-hover:text-foreground transition-colors">
                  {step}
                </p>
              </div>
            ))}
