...
const BLOCKING_REASON_LABELS: Record<string, string> = {
  oauth_app_not_configured: 'Configuração OAuth da plataforma ausente.',
  webhook_secret_not_configured: 'Segredo de webhook não configurado.',
  mercadopago_connection_not_connected: 'Conta Mercado Pago não está conectada.',
  webhooks_disabled: 'Webhooks estão desativados.',
  payment_topic_disabled: 'Tópico de pagamento está desativado.',
};
    ...
    if (typeof window !== 'undefined') {
      ...
      if (urlParams.get('error') === 'mp_not_configured') {
        toast.error('Configurações pendentes', {
          description: 'A plataforma ainda não terminou a configuração global do Mercado Pago.',
          duration: 10000,
        });
      ...
      if (urlParams.get('error') === 'mp_oauth_failed') {
        toast.error('Falha na autenticação', {
          description: 'Não conseguimos verificar sua conta no Mercado Pago. Tente novamente em instantes.',
          duration: 10000,
        });
        window.history.replaceState({}, document.title, window.location.pathname);
      }
    ...
    try {
      ...
      if (error instanceof ApiClientError && error.status === 409) {
        ...
        try {
          ...
          // noop: fallback to current local state when admin settings refresh fails.
        }
      } else {
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <Card className="h-fit">
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between gap-4">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Checkout URL ativa: {adminSettings?.effectiveCheckoutUrlKind === 'init_point' ? 'init_point' : 'sandbox_init_point'}
                  </p>
                </div>
                <Switch
              ...
              {form.payment_mode === 'production' && adminSettings && !adminSettings.productionReady && (
                <div className="space-y-2 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-[10px] leading-relaxed text-destructive">
                  <p className="font-semibold uppercase tracking-wide">Bloqueios para produção</p>
                  <ul className="space-y-1">
                    {(adminSettings.blockingReasons || []).map((reason) => (
                      <li key={reason}>- {formatBlockingReason(reason)}</li>
                    ))}
                  </ul>
                </div>
              )}
