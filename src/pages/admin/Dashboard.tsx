...
const RevenueByDayChart = lazy(() =>
  import('@/pages/admin/dashboard/RevenueByDayChart').then((module) => ({
    default: module.RevenueByDayChart,
  }))
);

const PaymentMethodsChart = lazy(() =>
  import('@/pages/admin/dashboard/PaymentMethodsChart').then((module) => ({
    default: module.PaymentMethodsChart,
  }))
);
    ...
    async function loadDashboard() {
      try {
        ...
        if (tenantSlug) {
          ...
          const summary = snapshot?.dashboardSummary as
            | { recipes?: RecipeRecord[]; payments?: Payment[] }
            | undefined;
          if (summary) {
    ...
    return Object.entries(counts)
      .map(([slug, data]) => {
        ...
        return {
          slug,
          name:
            recipe?.title || slug.replace(/-/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase()),
  ...
  return (
    <>
      <PageHead
        ...
        description="Acompanhe receitas, pagamentos e métricas do seu negócio."
        noindex={true}
      />
      <div className="space-y-8 pb-10">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="group relative overflow-hidden rounded-3xl border bg-card p-6 shadow-sm transition-all hover:shadow-md hover:border-primary/20"
            >
              <div className="flex items-center justify-between">
                <div
                  className={`rounded-2xl bg-muted/50 p-3 ${stat.color} transition-colors group-hover:bg-primary/10`}
                >
              ...
              <div
                className={`absolute bottom-0 right-0 h-24 w-24 translate-x-8 translate-y-8 rounded-full bg-primary/5 opacity-0 transition-opacity group-hover:opacity-100`}
              />
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="rounded-3xl border bg-card p-6 shadow-sm overflow-hidden flex flex-col">
            ...
            <Link to={buildTenantAdminPath('receitas', tenantSlug)} className="mt-6">
              <Button
                variant="outline"
                className="w-full rounded-2xl h-11 text-xs font-bold border-muted-foreground/20 hover:bg-muted/50"
              >
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                    Suas vendas aprovadas representam{' '}
                    {((approvedPayments.length / (payments.length || 1)) * 100).toFixed(1)}% do
                    total. Melhore seu checkout para aumentar esta taxa.
                  </p>
                </div>
              </div>
            </div>
