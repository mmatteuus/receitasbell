...
export default function PwaAuthVerifyPage() {
  ...
  const pageTitle = status === 'checking' ? 'Validando acesso' : errorTitle;
    ...
    async function run() {
      ...
      try {
        ...
        if (typeof navigator !== 'undefined' && !navigator.onLine) {
          ...
          setErrorMessage(
            'Você está offline. Conecte-se à internet para validar seu link de acesso.'
          );
          return;
        }
        ...
        <Card className="w-full max-w-sm border-none bg-transparent text-center shadow-none">
          ...
          <CardContent>
            <Button
              ...
              onClick={() =>
                navigate(
                  buildPwaPath('login', { tenantSlug: resolvePwaTenantSlug(location.pathname) }),
                  { replace: true }
                )
              }
            >
