...
export async function listCategories() {
  const result = await jsonFetch<{ categories?: Category[]; items?: Category[] }>(
    '/api/public/categories'
  );
  return result.categories ?? result.items ?? [];
}

export async function createCategory(input: { name: string; description?: string; icon?: string }) {
  const result = await jsonFetch<{ category?: Category; item?: Category }>(
