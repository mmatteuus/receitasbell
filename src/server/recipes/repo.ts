...
export async function getRecipeById(tenantId: string | number, id: string | number): Promise<RecipeRecord | null> {
  try {
    const record = await fetchBaserow<any>(`/api/database/rows/table/${BASEROW_TABLES.RECIPES}/${id}/?user_field_names=true`);
...
export async function createRecipe(tenantId: string | number, recipe: Partial<RecipeRecord>): Promise<RecipeRecord> {
  ...
  const record = await fetchBaserow<any>(`/api/database/rows/table/${BASEROW_TABLES.RECIPES}/?user_field_names=true`, {
      method: "POST",
...
export async function updateRecipe(tenantId: string | number, recipeId: string | number, recipe: Partial<RecipeRecord>): Promise<RecipeRecord> {
  ...
  const record = await fetchBaserow<any>(`/api/database/rows/table/${BASEROW_TABLES.RECIPES}/${recipeId}/?user_field_names=true`, {
      method: "PATCH",
      body: JSON.stringify(payload),
  });
