...
export async function upsertRating(tenantId: string | number, input: { recipeId: string; userId: string | undefined; value: number; authorEmail: string }): Promise<any> {
  ...
  if (existing.results[0]) {
    return await fetchBaserow<any>(`/api/database/rows/table/${BASEROW_TABLES.RATINGS}/${existing.results[0].id}/?user_field_names=true`, {
        method: "PATCH",
        body: JSON.stringify({ value: input.value, updated_at: now }),
    });
  }

  return await fetchBaserow<any>(`/api/database/rows/table/${BASEROW_TABLES.RATINGS}/?user_field_names=true`, {
      method: "POST",
