...
export async function getStripeConnectUrl(
  ...
  const tableId = BASEROW_TABLES.STRIPE_OAUTH_STATES;
  if (!tableId) throw new ApiError(500, "BASEROW_TABLE_STRIPE_OAUTH_STATES nao configurada.");
  ...
  await fetchBaserow(`/api/database/rows/table/${tableId}/?user_field_names=true`, {
    method: "POST",
    body: JSON.stringify({
...
export async function handleStripeOAuthCallback(
  ...
  const data = await fetchBaserow<{ results: OAuthStateRow[] }>(
    `/api/database/rows/table/${tableId}/?user_field_names=true&filter__state_hash__equal=${encodeURIComponent(stateHash)}`,
  );
  const row = data.results[0];
  if (!row) throw new ApiError(400, "Stripe OAuth state invalido ou expirado.");
  ...
  if (!row.expires_at || new Date(row.expires_at).getTime() <= Date.now()) {
    await fetchBaserow(`/api/database/rows/table/${tableId}/${row.id}/`, { method: "DELETE" });
