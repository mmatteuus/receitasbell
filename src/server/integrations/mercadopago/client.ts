...
async function fetchMP<T>(tenantId: string | number, endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (attempt < MAX_RETRIES && (response.status >= 500 || response.status === 429)) {
          logger.warn(`Mercado Pago temporary error (${response.status}). Attempt ${attempt}/${MAX_RETRIES}`, { endpoint });
          continue;
        }
        logger.error(`Mercado Pago API Error: ${response.status} ${endpoint}`, { body });
      ...
      if (attempt < MAX_RETRIES) {
        logger.warn(`Mercado Pago attempt ${attempt} failed: ${error.message}. Retrying...`, { endpoint });
        continue;
      }
    }
  }
