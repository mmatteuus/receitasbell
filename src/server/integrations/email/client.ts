...
export async function sendEmail(input: {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (!response.ok) {
        const error = (await response.json().catch(() => ({ message: "Unknown error" }))) as Record<string, unknown>;
      ...
      if (attempt < MAX_RETRIES) {
        ...
        logger.warn(`Email attempt ${attempt} failed: ${message}. Retrying...`, { to: input.to });
        await sleep(1000 * attempt);
        continue;
      }
    }
  ...
  throw new ApiError(502, `Email total failure: ${lastMessage}`);
}
