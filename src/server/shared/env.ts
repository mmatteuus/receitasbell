// Note: This file was moved to shared/env.ts
// Imports updated to reflect new relative structure if needed.
// For now, settingsRepo remains in ../baserow/ (until moved later)

import { getSettingsMap, mapTypedSettings } from "../integrations/baserow/settingsRepo.js"; 
...
export function validateCriticalEnv() {
  ...
  if (missing.length > 0) {
    throw new Error(`CRITICAL_BOOT_FAILURE: Missing environment variables: ${missing.join(", ")}`);
  }
}
